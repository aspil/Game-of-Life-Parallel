...
if [[ $res_ceil != $res_floor ]]; then
	echo "[$(whoami)]: $n_proc is not a power of 2. Please give a number that is a power of 2."
	exit
fi

# Remove the switch argument from mpirun command on mpiPBSscript.sh
sed -i -e 's/mpirun -n [0-9]* .*.x [0-9]*/mpirun '$executable'.x '$grid_sz'/g' mpiPBSscript.sh

if [[ n_proc -le 4 ]]; then
	sed -i -e 's/select=[0-9]*/select=1/g' mpiPBSscript.sh
	sed -i -e 's/mpiprocs=[0-9]*/mpiprocs='$n_proc'/g' mpiPBSscript.sh
	sed -i -e 's/mpirun .*.x [0-9]*/mpirun -n '$n_proc' '$executable'.x '$grid_sz'/g' mpiPBSscript.sh
else
	nodes=$(($n_proc / 8))
